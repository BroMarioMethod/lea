...
validate_arguments() {
    ...
    for argument in "$@"; do
        case "$argument" in
            ...
            *)
                printf \
                    'Unsupported uninstall wrapper argument: %s\n' \
                    "$argument" \
                    >&2
                printf 'Supported arguments: --yes, --help\n' >&2
                return 2
                ;;
        esac
    done
...
require_root() {
    ...
    if [[ "$user_id" != "0" ]]; then
        printf \
            'LEA uninstallation must be run as root. Use: sudo ./uninstall.sh\n' \
            >&2
        return 1
    fi
}
...
main() {
    ...
    exec "$uv_binary" run lea uninstall-release-candidate \
        --purge \
        "$@"
}
