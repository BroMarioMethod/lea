"""Tests for the Local CLI task-delete command."""
...
class RecordingProvider:
    """Record provider-neutral task-deletion calls."""
...
def test_task_delete_returns_canonical_deleted_task(tmp_path: Path) -> None:
    task = TaskRecord(
        uuid=TASK_UUID,
        description="Deleted CLI task",
    ...
    assert provider.deleted == [TASK_UUID]
    assert "Task deleted" in render_task_delete_result(result)


def test_task_delete_failure_maps_to_application_error(tmp_path: Path) -> None:
