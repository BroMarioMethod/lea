"""Tests for the Local CLI proposal-rejection command."""
...
DECIDED_AT = datetime(2026, 7, 22, 13, 0, tzinfo=UTC)
...
def _proposal(
    *, status: ActionStatus = ActionStatus.AWAITING_CONFIRMATION
    ...
    return ActionProposal(
        ...
        action="task.delete",
        parameters={"uuid": "44444444-4444-4444-8444-444444444444"},
        status=status,
        source="test",
        created_at=datetime(2026, 7, 22, 10, 0, tzinfo=UTC),
        reason="Delete one test task.",
    )
...
def test_reject_persists_audit_and_replacement(tmp_path: Path) -> None:
    ...
    assert proposal_data["status"] == "rejected"
    assert result.data["audit_persisted"] is True
    assert result.data["proposal_persisted"] is True
    assert result.data["actor"] == "Marius"
    assert result.data["reason"] == "Not authorised."
    assert "Proposal rejected." in render_proposal_reject_result(result)


def test_reject_allows_omitted_reason(tmp_path: Path) -> None:
    result = execute_proposal_reject(
        config_path=tmp_path / "lea.toml",
...
def test_reject_rejects_blank_actor_before_configuration() -> None:
    called = False
...
def test_reject_rejects_blank_supplied_reason() -> None:
    result = execute_proposal_reject(
        config_path=Path("/tmp/lea.toml"),
        expected_profile=None,
        proposal_id=PROPOSAL_ID,
...
def test_reject_rejects_invalid_proposal_state(tmp_path: Path) -> None:
    result = execute_proposal_reject(
        config_path=tmp_path / "lea.toml",
        expected_profile=None,
        proposal_id=PROPOSAL_ID,
